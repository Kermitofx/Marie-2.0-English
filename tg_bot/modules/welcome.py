...
WELC_HELP_TXT = "As mensagens de boas-vindas/despedida do seu grupo podem ser personalizadas de várias maneiras. Se você quiser as mensagens sejam personalizadas, você pode usar estas variáveis:
 - {first}: representa o nome do usuário
 - {last}: representa o sobrenome do usuário. Mostra o nome caso o usuário não tenha sobrenome.
 - {fullname}: representa o nome completo do usuário. Mostra o nome caso o usuário não tenha sobrenome.
 - {username}: representa o username. Cita o usuário caso ele não tenha username.
 - {mention}: apenas cita o usuário - mencionando seu primeiro nome.
 - {id}: representa o id do usuário.
 - {count}: representa o número de membro do usuário.
 - {chatname}: representa o nome do grupo.

As variáveis DEVEM ser escritas entre {}.

As mensagens de boas-vindas também suportam markdown, para que você possa formatar qualquer elemento em negrito/itálico/código/links. 
Os botões também são suportados, para que você possa dar as boas-vindas um visual incrível com alguns botões personalizados.

Para criar um botão com um link para as suas regras, use o seguinte formato: [Regras](buttonurl://t.me/encarregada_bot?start=group_id). Apenas substitua group_id pelo ID do seu grupo, que pode ser obtido com o comando /id. Observe que os IDs de grupos são precedidos por -; este sinal é obrigatório, então não o esqueça.

Se você estiver se sentindo espirituoso, poderá até mesmo definir imagens/gifs/vídeos/mensagens de voz como a mensagem de boas-vindas, respondendo à mídia desejada com o comando /setwelcome.
...
Apenas administradores:
 ...
 - /welcomehelp: ver mais informações sobre como formatar as mensagens personalizadas de boas-vindas/despedida.".format(dispatcher.bot.username)
